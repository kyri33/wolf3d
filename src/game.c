...
/*   Updated: 2016/11/13 17:52:32 by kioulian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_step(t_env *e, t_game *game)
{
	...
	{
		game->step_x = 1;
		game->side_dist.x = (game->map_x + 1.0 - e->ray_pos.x)
			* game->delta_dist.x;
	}
	if (e->ray_dir.y < 0)
	{
	...
	{
		game->step_y = 1;
		game->side_dist.y = (game->map_y + 1.0 - e->ray_pos.y)
			* game->delta_dist.y;
	}
}
...
void	perform_dda(t_env *e, t_game *game)
{
	...
	if (game->side == 0)
		game->wall_dist = (game->map_x - e->ray_pos.x +
				(1 - game->step_x) / 2) / e->ray_dir.x;
	else
		game->wall_dist = (game->map_y - e->ray_pos.y +
